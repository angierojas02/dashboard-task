 ...
 export function Target ({id, title, description, priority, status, onDelete, onModify}) {
    ...
    return (
        <div className='shadow md:max-w-80 rounded-2xl max-w-72 m-2.5 bg-slate-800 overflow-hidden text-white'>
            <div className="p-3 flex flex-col gap-1">
                <h3 className="text-base mb-1 font-bold">{title.toUpperCase()}</h3>
                <p className="text-sm mb-1">{description}</p>
                <button onClick={() => onModify(id, nextStatus)} className={`font-bold bg-amber-50 rounded-2xl ${colorStatus[status]}`}>{buttonText}</button>
